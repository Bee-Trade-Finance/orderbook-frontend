...
const TradingChart = memo(({theme, width, pair, pairData}) => {
  ...
  useEffect(() => {
    const chartOptions = { 
      width: document.querySelector('.main-chart').clientWidth, 
      ...
      layout: { 
        ...
        background: { type: 'solid', color: theme==='dark'?  '#131722' : 'white' },
      } 
    };
    document.querySelector('.main-chart').innerHTML = "";
    const chart = createChart(document.querySelector('.main-chart'), chartOptions);
  ...
  }, [theme, document.querySelector('.main-chart')?.clientWidth? document.querySelector('.main-chart').clientWidth : 0 ])

  return (
    <>
    <div className='row'>
      <div className='col-4'>
          <p className="img-fluid m-0 p-0"><img className="img-fluid m-0 p-2" src={logos[pair[0]]} style={{height: '40px'}} /></p>
          <p className='text-warning m-0 p-0'>{pair[0]} - {pair[1]}</p>
      </div>
      <div className='col-4'>
